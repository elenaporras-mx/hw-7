...
class ConditionalRandomField(HiddenMarkovModel):
    ...
    def train(self,
        ...
        for step, mb in enumerate(evalbatch):
            # Existing training loop code...
            
            # Add checkpoint saving after parameter update
            if save_path:
                self.save(save_path, checkpoint=step)
    ...
    def save(self, path: str, checkpoint: Optional[int] = None) -> None:
        """Save the model parameters to a file at the given path.
        
        Args:
            path: Path to save the model parameters
            checkpoint: Optional step number for checkpoint saving
        """
        if checkpoint is not None:
            # Create checkpoint filename by adding step number before extension
            base, ext = os.path.splitext(path)
            path = f"{base}.step{checkpoint}{ext}"
        ...
        if checkpoint is not None:
            state_dict['checkpoint_step'] = checkpoint
